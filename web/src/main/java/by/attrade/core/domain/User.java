...
import by.attrade.core.converter.PasswordConverter;
import by.attrade.core.type.Role;
...
public class User implements UserDetails, Serializable {
    ...
    private Long id;
    @Length(min = 3, max = 24, message = "{by.attrade.core.domain.User.username.error}")
    private String username;
    @Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,}$",
            message = "{by.attrade.core.domain.User.password.error}")
    ...
    @Email(regexp = "(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&" +
            ...
            "\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])",
            message = "{by.attrade.core.domain.User.email.error}")
